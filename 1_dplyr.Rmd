...
# gentoo_ladies = gentoo[gentoo$species == "Gentoo" & gentoo$sex == "female", ] # vs. base R (note, this retains NAs)
```
...
The `dplyr` syntax is simpler, and the name of the original dataset (`penguins`) does not need to be repeated when using `filter()`, unlike when using `which()`. Base R also requires the creation of an interim variable, or if you want it to all fit into one line of code, you must subset across rows and columns simultaneously and then `unlist()` your `data.frame` or `tibble` so that it can be read into the `mean()` function as an array. 
